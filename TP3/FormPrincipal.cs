...
namespace TP3
{
  public partial class FormPrincipal : Form
  {
    ...
    TypeEtat pieceTableau;  //<aouellet> la pièce utilisée présentement par le tableau
    ...
    //<aouellet>
    /// <summary>
    /// La fonction ExecuterTestRetirerLigneA est un test qui rempli une ligne du tableau avec des blocs gelés. Ensuite, elle efface la ligne et vérifie si la ligne est bien effacée.
    /// </summary>
    void ExecuterTestRetirerLigneA()
    {
      ...
      List<int> lignesCompleterTest = new List<int>(); //<aouellet>La liste des lignes complétés pour les fonctions Effacer et Décaler
      ...
      btnStart.Enabled = true;                                        //<aouellet> ces lignes sont essentiellement l'option réinitialiser.
    ...
    /// <summary>
    /// La fonction ExecuterTestRetirerLigneB est un test qui rempli une ligne du tableau avec des blocs gelés et une autre avec des blocs non gelés.
    /// Ensuite, elle efface la ligne et vérifie si la ligne est bien effacée puis l'autre ligne décalle correctement.
    /// </summary>
    ...
    /// <summary>
    /// La fonction ExecuterTestRetirerLigneC est un test qui rempli deux lignes du tableau avec des blocs gelés.
    /// Ensuite, elle efface ces lignes et vérifie s'ils ont étés bien effacée.
    /// </summary>
    ...
    /// <summary>
    /// La fonction ExecuterTestRetirerLigneD est un test qui rempli deux lignes du tableau non consécutives avec des blocs gelés.
    /// Ensuite, elle efface ces lignes et vérifie s'ils ont étés bien effacée.
    /// </summary>
    ...
    /// <summary>
    /// La fonction ExecuterTestRetirerLigneE est un test qui rempli trois lignes du tableau avec des blocs gelés.
    /// Ensuite, elle efface ces lignes et vérifie s'ils ont étés bien effacée.
    /// </summary>
    ...
    /// <summary>
    /// La fonction ExecuterTestRetirerLigneE est un test qui rempli quatres lignes du tableau avec des blocs gelés.
    /// Ensuite, elle efface ces lignes et vérifie s'ils ont étés bien effacée.
    /// </summary>
    ...
    // <aouellet>
    /// <summary>
    /// La fonction effacer ligne reçoit en paramètre le nombre d'une ligne dans le tableau qui est a effacé puis l'efface dans le tableau des états et des images.
    /// </summary>
    /// <param name="nbDeLaLigneComplete"></param> Le paramètre nbDeLaLigneComplete est un int qui est le numéro d'une ligne donné par une autre fonction.
    ...
    //<aouellet>
    /// <summary>
    /// La fonction VerifierLigne vérifie tout le tableau pour voir si une ligne est complète (pleine de blocs gelés). Si c'est le cas, elle l'ajoute à une liste qu'elle retournera à la fin.
    /// </summary>
    /// <returns></returns> lignesCompleter, une liste de int, qui contient les nombres des lignes qui sont pleines de bloc gelés.
      ...
      for (int i = 0; i < tableauEtats.GetLength(0); i++)
      {
        for (int j = 0; j < tableauEtats.GetLength(1); j++)
        {
          if (tableauEtats[i, j] == TypeEtat.GELE)
          {
            ...
            if (compteurSiLigneComplete == tableauEtats.GetLength(1)) //Verifie si ligne est complète
            {
    ...
    //<aouellet>
    /// <summary>
    /// La fonction GeleBlocs fait en sorte que les blocs d'un bloc actif deviennent des blocs gelés lorsqu'il est appelé. De plus, il initialise une nouvelle pièce dans la tableau
    /// après avoir gelé le bloc.
    /// </summary>
    void GeleBlocs()
    {
      for (int i = 0; i < blocActifY.Length; i++)
      ...
      pieceTableau = GenererPieceAleatoire();                              //Réinitialise une pièce après le gel.
      ligneCourante = 0;
      colonneCourante = tableauEtats.GetLength(1) / 2 - 1;
      InitialiserPieceDansTableau(pieceTableau);
    }
    ...
    //<aouellet>
    /// <summary>
    /// La fonction EnleverAncienBloc s'assure qu'après un déplacement, l'endroit à la position précédente ne contient plus les états ou les images du blocs. Il les change à l'état
    /// de case vide.
    /// </summary>
    ...
    // <scloutier> & <aouellet>
    /// <summary>
    /// Le tick gère tous les évenements qui doivent commencer sur un certain tick. Cela inclut le déplacement d'un bloc automatique, le changement des statistiques et des 
    /// vérifications pour Effacer et Décaler les lignes. De plus elle gère aussi le score après avoir effacé une ligne. 
    /// </summary>
    /// <param name="sender"></param>
    /// <param name="e"></param>
    ...
    //<aouellet>
    /// <summary>
    /// La fonction DecalerLignes décale les lignes vers le bas du tableau à partir d'une certaine ligne reçu en paramètre. Ensuite il s'assure que des nouvelles lignes vides
    /// apparaissent au début du tableau.
    /// </summary>
    /// <param name="nbLigneADecaler"></param>
    ...
    //<aouellet>
    /// <summary>
    /// La fonction DessinerTableau change toutes les images visuelles selon l'état qu'ils sont dans le tableau État. La couleur du pictureBox dépends de quel état il s'agit.
    /// </summary>
